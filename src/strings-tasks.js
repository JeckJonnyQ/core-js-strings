...
function formatTime(minutes, seconds) {
  const minutesFull = String(minutes).padStart(2, '0');
  const secundesFull = String(seconds).padStart(2, '0');
  return `${minutesFull}:${secundesFull}`;
}
...
function isPalindrome(str) {
  const takeOnlyStr = str.replace(/[^A-Za-z]/g, '').toLowerCase();
  const reverseStr = takeOnlyStr.split('').reverse().join('');
  if (reverseStr === takeOnlyStr) {
    return true;
  }
  return false;
}
...
function findLongestWord(sentence) {
  const sentenceMassive = sentence.split(' ');
  let longestWord = '';
  for (let i = 0; i < sentenceMassive.length; i += 1) {
    if (sentenceMassive[i].length > longestWord.length) {
      longestWord = sentenceMassive[i];
    }
  }
  return longestWord;
}
...
function reverseWords(str) {
  const strMassive = str.split(' ');
  const result = [];
  for (let i = 0; i < strMassive.length; i += 1) {
    result.push(strMassive[i].split('').reverse().join(''));
  }
  return result.join(' ');
}
...
function encodeToRot13(str) {
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    const takeCharCode = str.charCodeAt(i);
    if (takeCharCode >= 65 && takeCharCode <= 90) {
      result += String.fromCharCode(((takeCharCode - 65 + 13) % 26) + 65);
    } else if (takeCharCode >= 97 && takeCharCode <= 122) {
      result += String.fromCharCode(((takeCharCode - 97 + 13) % 26) + 97);
    } else {
      result += str[i];
    }
  }
